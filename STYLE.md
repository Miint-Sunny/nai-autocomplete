...
写词页的正文是对话流（`.nai-agent-thread`），两种消息**形不同**：

- **用户消息**：右对齐小卡（`max-width: 86%`、`radius-lg`、`chip-bg` + `chip-shadow`）——
  它是「一句话」，配得上一颗胶囊卡。
- **回复不包壳**：散文直接排在玻璃上，提示词块沿用 `.nai-agent-block`（`.nai-history-item` 规格）。
  回复里已经有卡片了，外面再套一层就是卡中卡。
- 每块的动作**填入在前、复制在后**，收在块头右侧；「全部填入」是整条回复的次要动作，靠右收尾。
- 输入区在最底：textarea + 「发送」占满整行（`.nai-md3-actions` 打底拿主按钮配方），
  「清空」小号（`6px 10px` / 11px，同块卡片上的小按钮）贴最右。
- thread `flex: 1 0 auto` 吃掉写词页的剩余高度，把输入区顶在底部；短对话时空态一屏放得下
  （输入框 `min-height: 44px`、`.nai-agent` 纵向 gap 10px —— 这两个数压过一次 52px 的溢出）。
...
把悬浮窗还给出图页时，JS 那边的重定向拆干净了，界面还是不出现 —— 因为另有一条

```css
.nai-md3-root[data-novelai-image-page="true"] .nai-md3-panel { display: none; }
```

**改「某个界面在某种场景下不出现」时，JS 和 CSS 两边都要搜一遍。**
搜 `data-` 属性名（这里是 `novelaiImagePage` / `novelai-image-page`），
JS 写驼峰、CSS 写连字符，一次 grep 搜不全两种写法。

### 6.6 断点里别把控件的功能抽掉

工作台侧栏在 `max-width: 980px` 下被无条件压成 58px 图标条，
于是「收起/展开」在那个断点里点了什么都不会变 —— 用户看到的就是「有收起没展开」。

当时的处理是**把按钮藏掉**，这是把功能去掉而不是修好。正确做法是把条件从
「窗口窄」改回「用户收起了」（`[data-sidebar-collapsed]`），只把内容列让窄一点；
真正窄到抽屉≈整屏（560px）时才固定成图标条，也只有那里该藏按钮。

**响应式断点是调尺寸的，不是调功能的。**

### 6.7 阴影浓度按背景挑，不是越浓越高级
...
### 6.8 按序号写死的布局规则，加一个控件就会错位

反推动作行是两列网格，曾经有一条

```css
/* 反推区是五个按钮，最后那个「送去改词」独占一行 */
.nai-md3-actions-reverse button:nth-child(5) { grid-column: 1 / -1; }
```

加了「打开图片」之后变成六个：跨整行的成了**第五个**「包裹代码框」，
最后一个反而落单，右边空一格。规则本身没坏，是它依赖的数量变了。

六个按钮两列正好三行满，这条规则直接删掉就对了。**加减控件时要回头数一遍
有没有按序号写死的规则**（`nth-child` / `last-child` / `:only-child`），
它们不会报错，只会悄悄错位。
...
这件事被藏了**两层**：JS 的重定向拆掉之后，还有一条
`[data-novelai-image-page="true"] .nai-md3-panel { display: none }` 在 CSS 里（见 6.5）。
...
「提示词格式」（V5 / V4.5）在提示词段的段首 —— 它决定这一段里预设的默认值，得先看到它。
...
- **说明文字贴着它解释的那组。** 外观段曾经把悬浮球说明和玻璃说明一起堆在段末；玻璃开关还夹在
  三个悬浮球开关中间，和它的强度滑块隔开。
- **反馈类的状态条必须在所有页面可见。** 面板的 `.nai-md3-status` 曾经嵌在「反推」页 section 里 ——
  写词页和设置页的所有 setStatus（错误、「已填入 N 个 Character 栏」、「设置已保存」）根本无处显示，
  点了按钮就像没反应。现在它在页签轨下方、属于整个面板，`:empty { display: none }` 不占高度
  （工作台那条同款）。初始也不再塞「就绪」占位 —— 反推页自带快捷键提示行，重复的占位只会让
  每一页都顶着一条常驻横幅。
...
**「开了没有」要看 `getComputedStyle(el).display`，不要看 `classList.contains('nai-hidden')`。**
类去掉了不等于看得见 —— 另一条 CSS 完全可能把它 `display: none`。
断言写在类上，测试会通过、界面照旧打不开。
