...
- **四档生成方式**：默认（tag 骨架 + 自然语言）/ 展开（多人分角色栏）/ 改写（整理现有提示词）/ 精简（优先准确 tag）
- **角色栏数量**可指定 0~6，指定了就要求模型正好输出那么多个角色栏
- 自动附上 **NovelAI V5 规则核对**（权重语法、两种透明、画面文字、`depthness` 等新 tag），换成自己的 skill 也生效，可在设置里关
...
3. 在「画面需求」里用中文描述想要的画面。**写到词库角色的名字或别名，就等于点名了这个角色** —— 记得勾上知识源里的「词库角色」
4. 需要迭代时勾上知识源里的「当前提示词」或「上一轮结果」—— 这时它只改 2~3 处并说明改了什么，而不是整体重写（切到「改写」档会自动帮你勾上）
5. 点「写提示词」，结果里的代码框可以直接复制、写入输入框、填进 Character 栏，或送去「改词」
