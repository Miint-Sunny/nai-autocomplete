...
- 手动选图模式
- **从文件夹打开本地图片**：反推页的「打开图片」，**下次点开还停在上次那个文件夹**；本地 NAI 原图照样直接读提示词，不花钱
- Pixiv 一类覆盖层场景的选图兼容
- 图像直链失败时的抓图兜底与截图回退
...
- skill 可导入、可编辑、可导出、可切换；内置那份只读且始终保留
- **导入统一成一个盒子**：选文件或者直接粘一段，都先落进同一个可编辑的文本域，当场告诉你会解析出什么，确认了再导入。skill 和酒馆预设两处**用的是同一个组件**
...
- 测试连接按钮
- 服务商预设 + 协议预设
- **Endpoint 填 base URL 就行** —— 和各家 SDK、酒馆一个口径，按协议自动补 `/chat/completions`、`/v1/messages` 之类的路径，补出来什么就摆在输入框下面
...
2. 使用以下任一方式选图：
   ...
   - 点“打开图片”从文件夹里挑一张本地图（下次点开还在上次那个文件夹）
3. 在设置页填写 LLM 配置
4. 点击“反推并复制”
5. 结果会写入面板并自动复制到剪切板
...
| 提示词 | 系统提示词、反推指令、角色替换模式、目标角色提示词、词库角色套用、导入酒馆（SillyTavern）预设 |
| Endpoint | 只填到 base URL 就行（`https://api.deepseek.com`），按协议自动补路径；下方会显示补出来的完整地址。自建网关可关掉 |
